...
void attaquerSupemon(Supemon *attaquant, Supemon *defenseur, Player *joueur, int isWild, int attackIndex)
{
    ...
    float chanceEsquive = (float)defenseur->evasion / (defenseur->evasion + attaquant->speed);
    if ((rand() % 100) / 100.0 < chanceEsquive) {
        if (isWild) {
            printf("Wild %s (Level %d, HP %d/%d) uses %s but %s's %s dodges the attack!\n",
                   attaquant->name, attaquant->level, attaquant->hp, attaquant->maxHp,
                   attack.name, joueur->name, defenseur->name);
        } else {
            printf("%s's %s (Level %d, HP %d/%d) uses %s but Wild %s dodges the attack!\n",
                   joueur->name, attaquant->name, attaquant->level, attaquant->hp, attaquant->maxHp,
                   attack.name, defenseur->name);
        }
        return; 
    }
...
void lancerCombat(Supemon *supemonJoueur, Supemon *supemonSauvage, Player *joueur)
{
    ...
    while (supemonJoueur->hp > 0 && supemonSauvage->hp > 0)
    {
        ...
        {
            printf("\n--- Wild Supémon's Turn ---\n");
            attaquerSupemon(supemonSauvage, supemonJoueur, joueur, 1, 0);
        }
    }
